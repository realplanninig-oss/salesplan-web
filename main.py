...
async def track_and_block_requests(request: Request, call_next):
    ...
    if path in ["/", "/survey", "/diagnostic", "/payment", "/payment/success", "/start-clients", "/funnel-7-days", "/consultation", "/subscribe"]:
...
async def start_clients():
    content = '''
<div class="hero">
    <h1>Продюсер экспертов с кейсами от 60 000 до 2 000 000 ₽ запустит продажи</h1>
    <p>Вы получите клиентов в течение 14 дней после внедрения плана.</p>
    <div style="margin-top: 30px;">
        <a href="/survey" class="btn btn-primary" style="font-size: 18px; padding: 16px 32px;" onclick="ym(108348240,'reachGoal','click_start_clients'); return true;">🔥 Получить план внедрения</a>
    </div>
</div>
...
<div class="hero">
    ...
    <div style="margin-top: 30px;">
        <a href="/consultation" class="btn btn-primary" style="font-size: 18px; padding: 16px 32px;" onclick="ym(108348240,'reachGoal','click_funnel_7'); return true;">🔥 Получить воронку за 7 дней</a>
    </div>
</div>
...
<div style="text-align:center; margin: 20px 0;">
    <a href="/consultation" class="btn btn-primary" style="font-size: 18px; padding: 16px 32px;" onclick="ym(108348240,'reachGoal','click_funnel_7_2'); return true;">🚀 Получить воронку</a>
</div>
<div style="background: #e8f0fe; border-radius: 20px; padding: 24px; text-align: center; margin-top: 30px;">
    <p style="font-size: 16px;">🎁 <strong>Бонус:</strong> 30-минутный разбор вашей воронки после заполнения анкеты – <a href="/consultation">записаться</a>.</p>
</div>
'''
    return HTMLResponse(content=render_page(content))
...
<div class="hero">
    ...
    <div style="margin-top: 30px;">
        <a href="/survey" class="btn btn-primary" style="font-size: 18px; padding: 16px 32px;" onclick="ym(108348240,'reachGoal','click_get_plan'); return true;">🔥 Бесплатно: диагностика перед запуском клиентов →</a>
    </div>
...
<div class="form-card" style="text-align: center;">
    ...
    <hr style="margin: 32px 0;">
    <div style="backgro# File: main.py — веб-приложение Salesplan с админ-дашбордом (исправленная версия с новыми лендингами и ожиданием отчёта)
...
# === СТАРЫЙ /start-clients УДАЛЯЕМ (комментарий, чтобы не было дубля) ===
# @app.get("/start-clients") ... удалён

# === ОСТАЛЬНЫЕ ЭНДПОИНТЫ (survey, diagnostic, payment и т.д.) БЕЗ ИЗМЕНЕНИЙ, КРОМЕ /check_premium_status И /payment/success ===
...
# НОВЫЙ ЭНДПОИНТ ДЛЯ ПРОВЕРКИ СТАТУСА ПРЕМИУМ-ОТЧЁТА
...
async def payment_success(user_id: str, amount: int = 490):
    ...
    # Единая логика: если отчёта нет или он в статусе generating – показываем ожидание
    if not existing_report or existing_report["status"] == "generating":
        # Убедимся, что запись отчёта существует (если нет – создаём)
        if not existing_report:
            conn = sqlite3.connect(DB_PATH)
            cursor = conn.execute("INSERT INTO reports (user_id, report_type, status) VALUES (?, 'premium', 'generating')", (user_id,))
            conn.commit()
            conn.close()
            # Запускаем генерацию, если ещё не запущена
            if biz and answers and DEEPSEEK_API_KEY:
                report_id = cursor.lastrowid
                asyncio.create_task(generate_premium_report_background(user_id, biz["name"], biz["description"], answers, report_id))
        return HTMLResponse(content=render_premium_waiting_page(user_id, amount))
...
<div id="clientsTab" class="table-container"><h3>💰 Клиенты, оплатившие премиум-план</h3><table id="clientsTable"><thead><tr><th>Дата</th><th>Телефон</th><th>Бизнес</th><th>Анкета</th><th>Отчет</th><th></th></thead><tbody></tbody>}</div>
<div id="diagnosticsTab" class="table-container" style="display:none"><h3>📝 Бесплатные диагностики</h3><table id="diagnosticsTable"><thead><tr><th>Дата</th><th>Бизнес</th><th>Анкета</th><th>Статус</th><th></th></thead><tbody></tbody>}</div>
<div id="consultationsTab" class="table-container" style="display:none"><h3>📞 Заявки на консультации</h3><table id="consultationsTable"><thead><tr><th>Дата</th><th>Телефон</th><th>Желаемое время</th></thead><tbody></tbody>}</div>
</div>
<script>
let clientsData=[];
...
async def admin_consultations(auth: bool =# File: main.py — веб-приложение Salesplan с админ-дашбордом (исправленная версия с новыми лендингами и ожиданием отчёта)
