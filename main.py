        ...
        if event == "payment.succeeded" and status == "succeeded":
            ...
            if user_id and amount in (1490, 1000):
                ...
                try:
                    ...
                    logger.error(f"Failed to update paid_at: {e}")
...
async def payment_confirm(request: Request):
    ...
    if user_id:
        ...
        row = cursor.fetchone()
        conn.close()
        amount = row[0] if row and row[0] else 490
        return RedirectResponse(url=f"/payment/success?user_id={user_id}&amount={amount}", status_code=303)
...
<div class="form-card" style="text-align: center;">
    ...
    <div class="bot-link-block">
        ...
        <a href="https://max.ru/id781407988795_bot" target="_blank" class="btn btn-primary" style="margin-top: 10px;" onclick="ym(108348240,'reachGoal','premium_purchase_success'); return true;">
            🔥 Перейти в MAX-чат
        </a>
    </div>
...
<div class="form-card" style="text-align: center;">
    ...
    <hr style="margin: 32px 0;"># File: main.py — веб-приложение Salesplan с админ-дашбордом (финальная версия)
...
async def create_yookassa_payment(
    ...
    try:
        ...
        if response.status_code in (200, 201):
            ...
            # ЗАПУСКАЕМ ГЕНЕРАЦИЮ ОТЧЁТА СРАЗУ ПОСЛЕ СОЗДАНИЯ ПЛАТЕЖА
            biz = get_business_data(user_id)
            answers = get_form_data(user_id)
            if biz and answers and DEEPSEEK_API_KEY:
                existing_report = get_report(user_id, "premium")
...
async def payment_webhook(request: Request):
    try:
        ...
        if event == "payment.succeeded" and status == "succeeded":
            ...
            if user_id:
                # Обновляем paid_at уже существующего отчёта (отчёт уже генерируется или готов)
                conn = sqlite3.connect(DB_PATH)
                conn.execute("""
                    UPDATE reports SET paid_at = CURRENT_TIMESTAMP
                    WHERE user_id = ? AND report_type = 'premium' AND status = 'ready'
...
async def payment_confirm(request: Request):
    ...
    # Сначала пробуем найти payment_id в параметрах
    payment_id = params.get("paymentId") or params.get("payment_id")
    ...
    # Если есть user_id, пробуем найти последний ожидающий платёж
    if user_id:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.execute("""
            SELECT yookassa_payment_id, amount FROM payments 
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        if data.get("status") == "succeeded":
                            ...
                            conn.commit()
                            conn.close()
                            logger.info(f"Payment {payment_id} succeeded after confirm check")
                            return RedirectResponse(url=f"/payment/success?user_id={user_id}&amount={amount}", status_code=303)
                except Exception as e:
                    logger.error(f"Failed to check payment status: {e}")
            
            # Если проверка не удалась, всё равно показываем страницу успеха
            return RedirectResponse(url=f"/payment/success?user_id={user_id}&amount={amount}", status_code=303)
...
<div class="form-card" style="text-align: center; background: linear-gradient(135deg, #fff 0%, #f8f9fa 100%);">
    ...
    <form action="/consultation/submit" method="post" id="consultationForm">
        ...
        <div class="form-group">
            ...
            <input type="tel" name="phone" placeholder="+7 (___) ___-__-__" required style="border-radius: 16px;" value="{user_phone}">
...
<script>
    ...
    async function loadClients() {
        ...
        data.clients.forEach(client => {
            ...
            </div></div><div class="detail-section"><strong>📄 Описание бизнеса:</strong><br>${client.business_description || '-'}</div><tr>`;
        });
    }
